 ...
 */
export function validateCurrentWeek(week: number): {
  valid: boolean;
  error?: string;
} {
  ...
  if (isNaN(weekNum)) {
    return {
      valid: false,
      error: `current_week must be a number, got: ${week}`,
    };
  }

  // Allow weeks 0-25: 0 (preseason), 1-18 (regular), 19-22 (playoffs), 23-25 (offseason)
  if (weekNum < 0 || weekNum > 25) {
    return {
      valid: false,
      error: `current_week must be between 0 and 25, got: ${weekNum}`,
    };
  }
...
export async function getOrCreateSeason(
  ...
  if (options?.currentWeek !== undefined && options.currentWeek !== null) {
    ...
    currentWeek = parsed;
    console.log(`[SeasonManager] Parsed currentWeek:`, {
      original: options.currentWeek,
      parsed: currentWeek,
    });
  } else {
    currentWeek = defaultWeek;
    console.log(`[SeasonManager] Using default currentWeek:`, {
      phase,
      defaultWeek,
    });
  }
  ...
  // Double-check: ensure it's an integer between 0 and 25
  if (!Number.isInteger(currentWeek) || currentWeek < 0 || currentWeek > 25) {
    return {
      season: {} as Season,
      created: false,
      error: `current_week must be an integer between 0 and 25, got: ${currentWeek} (type: ${typeof currentWeek})`,
    };
  }

  // First, try to get existing season
  let seasonQuery = supabase.from("seasons").select("*").eq("year", year);
  ...
  const { data: existingSeason, error: fetchError } =
  ...
  if (existingSeason) {
    console.log(
      `[SeasonManager] Season ${year} exists (id: ${existingSeason.id}, phase: ${existingSeason.phase}, active: ${existingSeason.is_active})`
    );
    return {
  ...
  console.log(
    `[SeasonManager] Creating season ${year} for save_game_id: ${saveGameId || "null"}`
  );
  ...
  if (currentWeek === undefined || currentWeek === null) {
    ...
    console.error(
      `[SeasonManager] CRITICAL: currentWeek is undefined/null! Using default for phase ${phase}`
    );
    ...
    console.warn(
      `[SeasonManager] currentWeek was a float ${currentWeek}, rounded to ${finalCurrentWeek}`
    );
  } else {
    // Try to parse it
  ...
  // Final validation - must be integer between 0 and 25
  if (
    !Number.isInteger(finalCurrentWeek) ||
    finalCurrentWeek < 0 ||
    finalCurrentWeek > 25
  ) {
    console.error(
      `[SeasonManager] CRITICAL: Invalid currentWeek after all processing!`,
      {
        original: currentWeek,
        type: typeof currentWeek,
    ...
    return {
      season: {} as Season,
      created: false,
      error: `Invalid current_week value: ${currentWeek} (final: ${finalCurrentWeek}). Must be an integer between 0 and 25.`,
    };
  }
  ...
  // Final clamp to ensure it's in range (allow 0-25 for offseason weeks)
  const clampedCurrentWeek = Math.max(0, Math.min(25, safeCurrentWeek));
  ...
  // One final clamp to be absolutely sure (allow 0-25)
  const finalInt = Math.max(0, Math.min(25, finalCurrentWeekInt));
  ...
  if (
    isNaN(currentWeekForDB) ||
    currentWeekForDB < 0 ||
    currentWeekForDB > 25
  ) {
    console.error(
      `[SeasonManager] CRITICAL: Invalid currentWeekForDB after all processing!`,
      {
        finalInt,
        currentWeekForDB,
  ...
  console.log(`[SeasonManager] Final seasonData.current_week:`, {
    ...
    inRange: currentWeekForDB >= 0 && currentWeekForDB <= 25,
  });

  // Final validation before insert - one more check (should never fail now due to clamping)
  if (
    !Number.isInteger(seasonData.current_week) ||
    seasonData.current_week < 0 ||
    seasonData.current_week > 25
  ) {
    console.error(
      `[SeasonManager] CRITICAL: Validation failed at final check despite clamping!`,
      {
        seasonData,
        original: currentWeek,
        final: finalCurrentWeek,
  ...
  console.log(
    `[SeasonManager] About to insert season data:`,
  ...
  if (createError) {
    // Log the raw error object first to see what we're dealing with
    console.error(`[SeasonManager] Insert error (raw):`, createError);
    console.error(
      `[SeasonManager] Insert error (stringified):`,
      JSON.stringify(createError, null, 2)
    );
    console.error(
      `[SeasonManager] Insert error (keys):`,
      Object.keys(createError)
    );
    console.error(
      `[SeasonManager] Insert error (toString):`,
      String(createError)
    );

    // Extract error properties safely - handle empty objects and various error formats
    const errorCode =
      createError.code ||
      (createError as any).code ||
      (createError as any)?.error?.code;
    const errorMessage =
      createError.message ||
      (createError as any).message ||
      (createError as any)?.error?.message ||
      String(createError);
    const errorDetails =
      createError.details ||
      (createError as any).details ||
      (createError as any)?.error?.details;
    const errorHint =
      createError.hint ||
      (createError as any).hint ||
      (createError as any)?.error?.hint;

    // If error is empty object, try to get more info from the error string representation
    const errorString = String(createError);
    const isEmptyObject =
      errorString === "[object Object]" &&
      Object.keys(createError).length === 0;

    console.error(`[SeasonManager] Insert error (parsed):`, {
      code: errorCode,
      ...
      inRange: seasonData.current_week >= 0 && seasonData.current_week <= 25,
    });

    // If error is empty or unclear, check if it's a duplicate by trying to fetch the season
    // This handles cases where Supabase returns an empty error object for duplicate key violations
    if (
      isEmptyObject ||
      !errorCode ||
      !errorMessage ||
      ...
      console.log(
        `[SeasonManager] Empty or unclear error object, checking if season already exists (possible duplicate key)...`
      );

      // Try to fetch the season that might already exist
      const { data: existingSeasonCheck, error: fetchCheckError } =
        await seasonQuery.single();

      if (!fetchCheckError && existingSeasonCheck) {
        console.log(
          `[SeasonManager] Season ${year} already exists (id: ${existingSeasonCheck.id}) - treating as duplicate key`
        );
        return {
          season: existingSeasonCheck as Season,
          created: false,
        };
      }
    }
    ...
    if (
      ...
      errorMessage?.includes("seasons_current_week_check")
    ) {
    ...
    // Check if it's a duplicate key error (race condition or existing record)
    // PostgreSQL error code 23505 is for unique constraint violations
    if (
      errorCode === "23505" ||
      errorCode === "PGRST301" || // PostgREST duplicate key error
      ...
      console.log(
        `[SeasonManager] Duplicate key detected for season ${year}, fetching existing record...`
      );

      // Fetch the season that already exists
      ...
      if (raceError || !raceSeason) {
        return {
          season: {} as Season,
          created: false,
          error: `Failed to fetch existing season: ${raceError?.message || "Unknown error"}. Original error: ${errorMessage}`,
        };
      }

      console.log(
        `[SeasonManager] Retrieved existing season ${year} (id: ${raceSeason.id})`
      );
      return {
    ...
    // Other error
    console.error(
      `[SeasonManager] Error creating season ${year}:`,
      createError
    );
    return {
      season: {} as Season,
      created: false,
      error: `Failed to create season: ${errorMessage || "Unknown error"} (code: ${errorCode || "none"})`,
    };
  }
  ...
  console.log(
    `[SeasonManager] Successfully created season ${year} (id: ${createdSeason.id})`
  );
  return {
...
export async function updateSeasonPhase(
  ...
  // Double-check: ensure it's an integer between 0 and 25
  // Offseason weeks can be 23-25 (Resign, Free Agency, Draft)
  if (!Number.isInteger(parsedWeek) || parsedWeek < 0 || parsedWeek > 25) {
    return {
      success: false,
      error: `current_week must be an integer between 0 and 25, got: ${parsedWeek}`,
    };
  }
  ...
  const phaseWeekMap: Record<SeasonPhase, { min: number; max: number }> = {
    ...
    offseason: { min: 23, max: 25 }, // Offseason weeks: 23 (Resign), 24 (FA), 25 (Draft)
  };

  const phaseRange = phaseWeekMap[phase];
  if (parsedWeek < phaseRange.min || parsedWeek > phaseRange.max) {
    console.warn(
      `[SeasonManager] Warning: Week ${parsedWeek} is outside expected range for phase ${phase} (${phaseRange.min}-${phaseRange.max})`
    );
  }
  ...
  if (updateError) {
    console.error(
      `[SeasonManager] Error updating season ${year} phase to ${phase}:`,
      updateError
    );
    return {
      success: false,
  ...
  console.log(
    `[SeasonManager] Successfully updated season ${year} to phase: ${phase}, week: ${week}`
  );
  return { success: true };
}
...
export async function deactivateSeason(
  ...
  if (deactivateError) {
    console.error(
      `[SeasonManager] Error deactivating season ${year}:`,
      deactivateError
    );
    // Don't fail - deactivation is not critical
    console.warn(
      `[SeasonManager] Warning: Failed to deactivate season ${year}, but continuing...`
    );
    return {
      success: false,
...
export async function getActiveSeason(
  ...
  if (error && error.code !== "PGRST116") {
    console.error(
      `[SeasonManager] Error fetching active season ${year}:`,
      error
    );
    return {
      season: null,
