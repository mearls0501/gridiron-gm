...
| **`tails.milestonesOff` — the Poisson repair is PRESCRIBED, SPECIFIED and NOT YET DONE.** The guard counts how many of 49 threshold categories sit outside their NFL band, and at 16 seasons the finest representable rate is 1/16 = 0.0625, above the ≤0.06 pass line for seven of them — so **~2.75 failures per seed come from arithmetic** and a correctly calibrated sim fails them by construction (HANDOFF, `milestonesOff` quantization). The fix is a count-based Poisson-interval verdict: λ = NFL rate × seasons, pass inside the central 95% interval. It was scoped for task/311 and **not implemented — the session ran out of context and a rushed verdict change plus a re-locked baseline would have been worse than leaving it**. Panel readings for continuity: 14.4 (pre-availability), 17.2, 16.4, 20.8 (current). Pooled evidence says only two categories are genuinely elevated — 1,900+ receiving yards at 2.25x and 23+ sacks at 3.12x. Do not tune against this number. | 20.8 | 0 |
