...
| **`statcheck.qb20PassYds` — starter availability.** Sim starters never miss games, so every rank below the leader carries a full season's volume where the real ones do not: only 45% of real leading passers appear in 16+ games and 53% of real team-seasons have their leading passer under 90% of team attempts (`nfl-reference.md` §5.3). Baseline is CORRECT — the sim is wrong. Design pass pending; do not widen. | 3822 | 3046 ±244 |
| **`statcheck.rb5RushYds` — starter availability**, same cause as `qb20PassYds`. `CARRY_SHARE[0]` is 0.60 against a real leading-rusher share of 47.4%, but the live path is `script.leadBackShare` and per-game share legitimately exceeds season share once absences exist — so this is re-measured AFTER the availability pass, not recentred before it. Baseline is CORRECT; do not widen. | 1508 | 1191 ±95 |
| **`statcheck.wr10RecYds` — starter availability**, same cause, but red LOW and **possibly single-seed noise**: receiving read 0.99x of real at this rank over a 4-season run and only fails on the fast tier's single seed. Confirm against a panel before treating it as real. Baseline is CORRECT; do not widen. | 1048 | 1208 ±97 |
