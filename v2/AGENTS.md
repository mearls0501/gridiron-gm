...
| **`statcheck.rb5RushYds` — `CARRY_SHARE` EXONERATED (task/306, §5.5); the mix was the cause and is fixed.** The dials are right on the denominator they control (lead back 59.7% of RB carries against a real 58.3%; 68.4% within a game against a real 70.4%) — §5.3's 47.4% is a share of TEAM carries and RBs take only 80.7% of those. Adding receiver carries and kneel-downs (task/309) took this **1,455 → 1,304**. The residue is not distribution: §5.8 shows real run-heavy clubs do not spread carries either, so the remaining ~1.5% sits with the rushing tail's own production curve. Baseline is CORRECT; do not widen, do not touch `CARRY_SHARE`. | 1304 | 1191 ±95 |
