...
| **`drift.passRecordSeasons` — REGRESSION of 2026-07-29, largely retired 2026-08-02. And the guard is measuring the wrong total (task/307).** Fixing `cutWorstSurplus` kept high-potential young players alive, more of them reached their ceiling, and the 5,477-yard record fell more often (10 → 13 of 20). Starter availability took it to 8.6 of 20; the within-game QB rotation of task/307 took it to **7.0 of 20** (per seed 4/10/5/7/9 against 7/11/7/8/10 on the same seeds), inside the `max: 10` guard. §6.4's floor holds comfortably — the minimum seed reads 4, nowhere near "never 0". **But `drift` advances to `offseason-recap` before reading the stat line, and playoff games write into it** (`applyGameStats` is called from `playoffs.ts`), so this counts REG + POST yards against Manning's REGULAR-SEASON 5,477: on 12 matched sim seasons the best passing season reads 5,316 with the playoffs and 4,735 without. Both the 8.6 and the 7.0 are inflated by roughly that much. Reconditioning it is a design decision — Matt. The ≤3 target is still open and is not an availability problem. | 7.0 of 20 | ≤ 3 of 20 |
...
| **`statcheck.qb20PassYds` — the within-game half is now fixed; what is left is availability and play count. Re-measured 2026-08-02 (task/307).** Instrumented against all three candidates before editing. **Per-attempt yardage is EXONERATED** — ranks 11-20 run 7.11 YPA against a real 7.23, so every yard of the residual is attempts. **Within-game rotation was the convicted mechanism and the engine had it backwards:** the sim's leader took 98.8% of his club's attempts in games he played against a real 97.0% (§5.4b, 17-game era), because `onField` rested only the side that was AHEAD — real clubs go to the backup MORE readily when losing (91.1% share in a 25-point loss against 95.6% in a 25-point win), and the trailing side carries 72% of all attempts real starters do not take (86% of everything above the one-score-win floor). Fixed and fitted to the measured share (sim now 96.6%): **60-seed 3,431 → 3,278; gate 5-seed panel 3,413 → 3,358.** Note the guard cannot resolve its own fix at five seeds — paired per-seed deltas are +85/−206/+195/−42/−308, sd 194. **Widening the team attempt distribution was tested and REVERTED**: real defect (sd 36 against 60) but it moves this metric less than the noise while pushing `rushers1700` 0.8 → 1.4 and `leadPassYds` +160. What is left, both measured, neither touched: QB1 availability (sim 15.0 games of 17 against a real 14.23 — worth ~−180, and §5.3's pooled 89.4% share median flatters the sim, the 17-game figure is 85.8%) and a league-wide +2.2% scrimmage play count (~−75). Baseline is CORRECT; do not widen. | 3358 | 3046 ±244 |
