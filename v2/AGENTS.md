...
| item | today | target |
|---|---|---|
| `drift.tradesPerSeason` — real league-wide volume is ~90 a year. **RE-LOCKED 2026-09-13 (Wave 3.7 Packet 2, Matt SIGNED).** Today is **≈65** (Mac Studio 5-seed panel ~64.8 at `2752729` / #77). The old **7.8** figure was a `trimLog` artifact: the harness counted `Trade:` log rows that the 4000 ceiling deleted from season 6 on. `Trade:` rows stay permanent for the GM history page; volume now reads `state.seasonCounters.tradesExecuted` (not the log). Internal `trades ≤ 20` P0 deleted. Baseline floor is `min: 30`. Do not retune volume knobs toward 60–120. | ≈65 | 60-120 |
| `careers.survivalMae` — **MEASURED 2026-08-31 (task/324).** The 8.8 / “rounds 3–6 wash out 11–16 points too fast” leftover is stale. Careers 24 / seed 12345 on current main was **6.81**; after `draftCapitalHold` camp rope through year 1 (R6–R7) and year 2 (R6): **5.94**. y3 vs `nfl-reference.md` §2.2: R3 89.6 vs 79.6 (too high), R4 76.8 vs 70.7, R5 58.3 vs 65.2, R6 45.6→**50.0** vs 53.6, R7 34.1→**40.9** vs 38.1. MAE is now early-round over-survival as much as late-round washout; a late-round-only hold cannot take it under 4 while R1–R3 sit 5–10 points high. | 5.94 | < 4 |
| `careers.careerLenMae` — **MEASURED 2026-08-31 (task/324).** The “R6/R7 median 1 and 0 vs 4 and 2” leftover is closed on the traced medians. Main was **2 and 1** (`careerLenMae` 0.86). After the same hold change: **4 and 2**. MAE **0.86→0.57**. Residue is R1/R2/R4 one–two seasons long (9/8/7 vs 8/7/5), which this packet must not shorten. | 0.57 | < 0.5 |
| ~~`careers.r1QbSharePct` — leftover after PR #7/#9 is POSITION_VALUE on the CPU board~~ **RE-LOCKED 2026-08-31 — `max: 16`, not a two-sided band.** `cpuBoardValue` reads `√POSITION_VALUE` (PR #10; QB ≈ 1.84× a safety); contracts / trades / generation keep the raw 3.4×. Careers 30 / seed 12345: **15.1% → 11.6%** toward nfl 10.3 (`nfl-reference.md` §2.4); 2-seed panel (24 seasons): **9.77**. Other groups on the 30-season run: OL 20.3% (real 20.3), RB 2.4→ toward 4.2, LB 4.9→ toward 7.7, DB 19.8 (was 21.0 vs 16.7), WR 16.7 (was 13.7 vs 13.4), DL 20.0 (was 24.3 vs 24.5). Old lock 15.9 ±3.2 failed 11.6 / 9.77 on the floor while they sat on the `nfl` note. That band's ceiling (19.1) existed to catch a relapse to 19.6. New lock is a max: 11.6 / 9.77 / ~10.3 pass, 19.6 fails. Do not start the R1 QB never-start 22.4 vs 10.7 packet. | 11.6 / 9.77 ✓ | 10.3% |
...
| ~~`statcheck.rb5RushYds` — cause found and mostly fixed; residue is not distribution.~~ **ROW RETIRED 2026-08-31 — GREEN on the 5-seed panel.** `CARRY_SHARE` stays EXONERATED (§5.5 / §5.8). Volume was exonerated after task/309; residue was top-end YPC (+6.9% at rank 5, §5.10), not carry count. `runPlay` now flattens the RB skill slope (0.049 → 0.024) around the **live** mean of the 32 depth-chart lead backs (measured ~78, not the hypothesized 70 and not a hardcoded 60), recomputed each game so it stays a redistribution as ratings age. Jet sweeps / keepers keep the old lever. Panel **1357 → 1254** (1269 / 1205 / 1328 / 1267 / 1203) inside 1191 ±95. Fast single-seed 1327 is stream noise, same family as the inherited `qb5` / `wr10` reds. Baseline not moved. Do not touch `CARRY_SHARE`; do not chase `rushers1700` by spreading carries. | 1254 ✓ | 1191 ±95 |
...
| ~~`drift.saveGrowthMbPerSeason` — false P0 from a threshold conflict~~ **RESOLVED 2026-08-03; RE-LOCKED 2026-09-15 (Wave 3.9 Packet 2, Matt SIGNED).** Mac Studio 5-seed panel at `748036a` (#88) read **0.56**; signed max is **0.61** (panel + 0.05). `saveMbAtEnd` signed max **15.89** (panel 14.89 + 1.0 MB). Reason: PS/IR/waiver/camp-90 bodies plus Wave 1/2 fields on the encoded save (`nfl-reference.md` §6.9). **20 MB quota unchanged.** Prior Wave 3.7 lock was 0.52 / 13.1 at `2752729` / #77. | +0.56 MB ✓ | < 0.61 |
...
**Ratified 2026-09-15 (Matt, Wave 3.9 Packet 2).** Mac Studio
`gate:full:serial` at `748036a` (#88), 5-seed means:
`drift.saveMbAtEnd` max **15.89** (panel 14.89 + 1.0 MB);
`drift.saveGrowthMbPerSeason` max **0.61** (panel 0.56 + 0.05);
moved `statcheck` targets to panel (`qb5PassYds` **4089.8** ±360,
`leadPassYds` **4730** ±700, `qb10PassYds` **3710** ±322,
`leadTackles` **165.2** ±40, `maxGameRecYds` **249.6** ±80).
20 MB quota unchanged. `capBustSeasons` 0.40 and
`franchiseTagsPerSeason` 16.62 are findings, not retuned
(Claude lane / leak-or-behaviour). `p0Failures` and
`milestonesOff` not re-locked.

**Ratified 2026-09-13 (Matt, Wave 3.7 Packet 2).** Mac Studio
`gate:full:serial` at `2752729` (#77), 5-seed means:
`drift.saveMbAtEnd` max **13.1** (panel 12.10 + 1.0 MB);
`drift.saveGrowthMbPerSeason` max **0.52** (panel 0.47 + 0.05);
`drift.ovrDrift` **−1.70 ±1.5** (panel mean — the 53-man counts the
street body who replaced an injured starter, not the injured starter
at full OVR; that is the NFL); `drift.franchiseTagsPerSeason` **14 ±4**,
`nfl: 10`; `drift.tradesPerSeason` today ≈65, target 60–120, floor
`min: 30`. The old 7.8 / `trades ≤ 20` pair was a log-trim artifact.
20 MB quota unchanged. `capBustSeasons` 2.60 and
`minPayrollSeasonsUnder55` 4.40 are findings, not retuned.
