-- Migration: Clear all players from the players table to prepare for CSV reload
-- This removes all players including drafted players that were incorrectly added
-- After running this, use GameSetupWizard to create initial seed players (53 per team)
-- 
-- WARNING: This will delete ALL players and cascade to related records
...
-- This removes all team assignments for all save games
...
-- 7. Now delete all players (both seed players and incorrectly added drafted players)
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'Successfully cleared players table. You can now reload initial players using GameSetupWizard.';
  END IF;
END $$;

-- Note: After running this migration:
-- 1. Use GameSetupWizard to create a new game, which will generate initial seed players (53 per team)
-- 2. Or manually import your CSV file with initial players
-- 3. The base players table should only contain the initial 53 players per team (1,696 total)
-- 4. Drafted players will be added with team_id = NULL and tracked via player_team_assignments
