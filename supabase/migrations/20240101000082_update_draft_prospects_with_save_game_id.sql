-- Update draft_prospects table to require save_game_id
-- Since the table is empty, we can safely drop and recreate with the correct structure

-- Step 1: Drop the existing table (safe since it's empty)
DROP TABLE IF EXISTS public.draft_prospects CASCADE;

-- Step 2: Recreate the table with save_game_id as required
